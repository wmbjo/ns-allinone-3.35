...
MyRipHelper::MyRipHelper ()
{
  m_factory.SetTypeId ("ns3::Rip");
}

MyRipHelper::MyRipHelper (const MyRipHelper &o)
...
MyRipHelper*
MyRipHelper::Copy (void) const
{
  return new MyRipHelper (*this);
}

Ptr<Ipv4RoutingProtocol>
MyRipHelper::Create (Ptr<Node> node) const
{
...
void
MyRipHelper::Set (std::string name, const AttributeValue &value)
{
...
int64_t
MyRipHelper::AssignStreams (NodeContainer c, int64_t stream)
{
...
void MyRipHelper::SetDefaultRouter (Ptr<Node> node, Ipv4Address nextHop, uint32_t interface)
{
  Ptr<Ipv4> ipv4 = node->GetObject<Ipv4> ();
  NS_ASSERT_MSG (ipv4, "Ipv4 not installed on node");
  Ptr<Ipv4RoutingProtocol> proto = ipv4->GetRoutingProtocol ();
...
void
MyRipHelper::ExcludeInterface (Ptr<Node> node, uint32_t interface)
{
...
void MyRipHelper::SetInterfaceMetric (Ptr<Node> node, uint32_t interface, uint8_t metric)
{
