...
{
    std::cout << "Print Routing Table" << std::endl;
...
{
    std::cout << "Assign Streams" << std::endl;
...
{
      std::cout << "Deferred Route Output" << std::endl;
...
{
      std::cout << "Notify TX Error" << std::endl;
...
{
    std::cout << "Notify Add Address" << std::endl;
...
{
  std::cout << "Notify Remove Address" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
      std::cout << "Is My Own Address" << std::endl;
...
{
    std::cout << "Loopback Route" << std::endl;
...
{
      std::cout << "My Send Request" << std::endl;
  //NS_LOG_FUNCTION ( this << dst);
  // A node SHOULD NOT originate more than RREQ_RATELIMIT RREQ messages per second.
  // if (m_rreqCount == m_rreqRateLimit)
  //   {
...
RoutingProtocol::SendTo (Ptr<Socket> socket, Ptr<Packet> packet, Ipv4Address destination)
{
  ...
  socket->SendTo (packet, 0, InetSocketAddress (destination, AODV_PORT));
    std::cout << "SENDTO2" << std::endl;
}
void
RoutingProtocol::ScheduleRreqRetry (Ipv4Address dst)
{
    std::cout << "ScheduleRreqRetry" << std::endl;
  NS_LOG_FUNCTION (this << dst);
...
{
  std::cout << Simulator::Now().GetSeconds() << " Seconds --- " << "RECVAODV" << std::endl;
...
{
      std::cout << "Update Route Lifetime" << std::endl;
...
{
  std::cout << "UpdateRouteToNeighbor" << std::endl;
...
{
      std::cout << "Send Reply By Intermediate Node" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
      std::cout << "Recv Reply Ack" << std::endl;
  NS_LOG_FUNCTION (this);
  RoutingTableEntry rt;
...
{
  std::cout << "ProcessHello" << std::endl;
...
{
      std::cout << "RREQ Rate Limit Timer Expire" << std::endl;
  NS_LOG_FUNCTION (this);
  m_rreqCount = 0;
  m_rreqRateLimitTimer.Schedule (Seconds (1));
}
...
{
      std::cout << "Rerr Rate Limit Timer Expire" << std::endl;
  NS_LOG_FUNCTION (this);
  m_rerrCount = 0;
  m_rerrRateLimitTimer.Schedule (Seconds (1));
}
...
{
      std::cout << "Ack Timer Expire" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
      std::cout << "Send Packet From Queue" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
        std::cout << "SendRerrWhenNoRouteToForward" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
  std::cout << "SendRerrMessage" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
    std::cout << "FindSocketWithInterfaceAddress" << std::endl;
...
{
        std::cout << "DoInitialize" << std::endl;
  NS_LOG_FUNCTION (this);
  uint32_t startTime;
  if (m_enableHello)
    {
