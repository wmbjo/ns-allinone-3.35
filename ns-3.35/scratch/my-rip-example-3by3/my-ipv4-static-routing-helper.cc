...
NS_LOG_COMPONENT_DEFINE ("MyIpv4StaticRoutingHelper");

MyIpv4StaticRoutingHelper::MyIpv4StaticRoutingHelper()
{
}

MyIpv4StaticRoutingHelper::MyIpv4StaticRoutingHelper (const MyIpv4StaticRoutingHelper &o)
{
}

MyIpv4StaticRoutingHelper*
MyIpv4StaticRoutingHelper::Copy (void) const
{
  return new MyIpv4StaticRoutingHelper (*this);
}

Ptr<Ipv4RoutingProtocol>
MyIpv4StaticRoutingHelper::Create (Ptr<Node> node) const
{
  return CreateObject<MyIpv4StaticRouting> ();
}


Ptr<MyIpv4StaticRouting>
MyIpv4StaticRoutingHelper::GetStaticRouting (Ptr<Ipv4> ipv4) const
{
  NS_LOG_FUNCTION (this);
  Ptr<Ipv4RoutingProtocol> ipv4rp = ipv4->GetRoutingProtocol ();
  NS_ASSERT_MSG (ipv4rp, "No routing protocol associated with Ipv4");
  if (DynamicCast<MyIpv4StaticRouting> (ipv4rp))
    {
      NS_LOG_LOGIC ("Static routing found as the main IPv4 routing protocol.");
      return DynamicCast<MyIpv4StaticRouting> (ipv4rp); 
    } 
      ...
      for (uint32_t i = 0; i < lrp->GetNRoutingProtocols ();  i++)
        {
          ...
          if (DynamicCast<MyIpv4StaticRouting> (temp))
            {
              NS_LOG_LOGIC ("Found static routing in list");
              return DynamicCast<MyIpv4StaticRouting> (temp);
            }
        }
    }
...
void
MyIpv4StaticRoutingHelper::AddMulticastRoute (
  Ptr<Node> n,
  Ipv4Address source, 
  Ipv4Address group,
  Ptr<NetDevice> input, 
  NetDeviceContainer output)
{
  ...
  MyIpv4StaticRoutingHelper helper;
  Ptr<MyIpv4StaticRouting> Myipv4StaticRouting = helper.GetStaticRouting (ipv4);
  if (!Myipv4StaticRouting)
    {
      NS_ASSERT_MSG (Myipv4StaticRouting,
                     "Ipv4StaticRoutingHelper::SetDefaultMulticastRoute(): "
                     "Expected an Ipv4StaticRouting associated with this node");
    }
  Myipv4StaticRouting->AddMulticastRoute (source, group, inputInterface, outputInterfaces);
}
...
void
MyIpv4StaticRoutingHelper::SetDefaultMulticastRoute (
  Ptr<Node> n, 
  Ptr<NetDevice> nd)
{
  Ptr<Ipv4> ipv4 = n->GetObject<Ipv4> ();
  ...
  MyIpv4StaticRoutingHelper helper;
  Ptr<MyIpv4StaticRouting> ipv4StaticRouting = helper.GetStaticRouting (ipv4);
